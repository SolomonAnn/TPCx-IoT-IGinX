#!/bin/bash
source ./Benchmark_Macros_Machbase.sh
...
#MACHBASE_COORDINATOR_HOME=${COORDINATOR_HOME} machcoordinatoradmin --cluster-status | grep warehouse | awk -F'|' '{print $4","$6}' | tr -d ' ' | awk -F, '{if ($2 == "normal") count[$1]++;} END {for (j in count) print count[j]}' | sort -n | head -n 1

clush -w 10.100.250.109 '/data/machbase/coordinator/bin/machcoordinatoradmin --cluster-status' | grep warehouse | awk -F'|' '{print $4","$6}' | tr -d ' ' | awk -F, '{if ($2 == "normal") count[$1]++;} END {for (j in count) print count[j]}' | sort -n | head -n 1
