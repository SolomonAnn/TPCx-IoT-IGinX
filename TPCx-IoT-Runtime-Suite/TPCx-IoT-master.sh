...
then
   ...
   source ./IoT_cluster_validate_suite.sh $1 | tee -a ./TPCx-IoT-result-"$prefix".log
...
do
    ...
    recordcount=$(($num_records_per_client*$c))
    echo "recordcount="$recordcount >> ./confFiles/workloadiot-$c
    insertstart=$(echo "$insertstart+$num_records_per_client" | bc)
done
...
do
echo $k
clush -w $k -B "nohup $PWD/TPCx-IoT-client.sh $WARMUP_RECORDS_COUNT $prefix $i $k $DATABASE_CLIENT $PWD $NUM_INSTANCES_PER_CLIENT $NUM_THREADS_PER_INSTANCE $SUT_PARAMETERS workloadiot warmup > $PWD/logs/IoT-Workload-run-time-warmup$i-$k.txt" &
pids="$pids $!"
done
...
do
echo $k
clush -w $k -B "nohup $PWD/TPCx-IoT-client.sh $DATABASE_RECORDS_COUNT $prefix $i $k $DATABASE_CLIENT $PWD $NUM_INSTANCES_PER_CLIENT $NUM_THREADS_PER_INSTANCE $SUT_PARAMETERS workloadiot run > $PWD/logs/IoT-Workload-run-time-run$i-$k.txt" &
pids="$pids $!"
done
...
source ./IoTDataRowCount_$1.sh $i
...
database_binding="$DATABASE_CLIENT-binding"
md5sum ./TPCx-IoT-master.sh ./tpcx-iot/$database_binding/lib/core-0.13.0-SNAPSHOT.jar ./IoT_cluster_validate_suite.sh | tee -a $PWD/TPCx-IoT-result-"$prefix".log
echo "" | tee -a ./TPCx-IoT-result-"$prefix".log
