...
public class BasicDB extends DB {
  ...
  public Status scan(String table, String key, String client, String timestamp, Set<String> fields, long runStartTime,
                     Vector<HashMap<String, ByteIterator>> result1, Vector<HashMap<String, ByteIterator>> result2) {
    delay();

    if (verbose) {
      StringBuilder sb = getStringBuilder();
      sb.append("SCAN ").append(table).append(" ").append(key).append(" ").append(client).append(" ").append(timestamp).append(" [ ");
      if (fields != null) {
        for (String f : fields) {
          sb.append(f).append(" ");
        }
